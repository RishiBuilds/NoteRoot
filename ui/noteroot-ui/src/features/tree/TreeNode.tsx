...
export const TreeNode = React.memo(function TreeNode({
  ...
  return (
    <div>
      <div
        className={`flex cursor-pointer items-center rounded-base pb-1 pt-1 text-base transition-all duration-200 ease-in-out ${
          isActive
            ? 'bg-main text-main-foreground font-heading border-2 border-border shadow-shadow'
            : 'text-foreground hover:bg-main/20 border-2 border-transparent'
        }`}
      ...
      >
        <div className="flex flex-1 items-center gap-2">
          ...
          {!hasChildren && <File size={18} className="text-foreground/60" />}
          {hasChildren && <Folder size={18} className="text-foreground/60" />}
